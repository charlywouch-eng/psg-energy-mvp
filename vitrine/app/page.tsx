        ...
        <section className="bg-slate-50 border-b border-slate-200 py-16 lg:py-24">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="grid lg:grid-cols-2 gap-12 lg:gap-8 items-start">
              ...
              {/* Colonne droite — Simulateur statique (lien vers /eligibilite pour le formulaire complet) */}
                ...
                <div className="p-8">
                  <div className="mb-6">
                    <div className="flex items-center justify-between mb-2">
                      <span className="mono text-[11px] font-bold text-brand-emeraldDeep uppercase tracking-widest">
                        Étape 1 sur 3
                      </span>
                      <span className="mono text-[11px] text-slate-400">Diagnostic flash</span>
                    </div>
                    <div className="h-1.5 w-full bg-slate-200 relative">
                      <div className="h-1.5 bg-midnight-900" style={{ width: "33.33%" }}></div>
                    </div>
                  </div>

                  <h2 className="display text-2xl font-bold text-midnight-900 mb-1">
                    Quel est votre statut&nbsp;?
                  </h2>
                  <p className="text-sm text-slate-500 mb-6">
                    Le statut juridique de votre établissement détermine les guichets mobilisables.
                  </p>

                  <div className="space-y-3 mb-6">
                    {[
                      { value: "ehpad-prive", label: "EHPAD privé ou associatif", sub: "CEE tertiaire, fonds qualité" },
                      { value: "collectivite", label: "Collectivité territoriale (mairie, EPCI)", sub: "Écoles, crèches — Fonds vert & ACTEE·EduRénov" },
                      { value: "ehpad-public", label: "Établissement public de santé", sub: "EHPAD public autonome ou rattaché" },
                    ].map((opt) => (
                      <div key={opt.value} className="border-2 border-slate-200 p-4 flex items-center justify-between">
                        <div>
                          <div className="font-bold text-midnight-900 text-sm">{opt.label}</div>
                          <div className="text-xs text-slate-500 mt-1">{opt.sub}</div>
                        </div>
                        <div className="w-4 h-4 rounded-full border-2 border-slate-300 shrink-0" aria-hidden="true"></div>
                      </div>
                    ))}
                  </div>
                  ...
                  <p className="mt-5 flex items-center justify-center gap-2 text-[11px] text-slate-400 font-medium text-center">
                    ...
                    Données strictement destinées à l&apos;étude technique interne — jamais transmises à un tiers commercial.
                  </p>
                </div>
              </div>
